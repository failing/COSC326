 ...
 * https://www.nayuki.io/page/smallest-enclosing-cc
 * 
...
public final class SmallestEnclosingCircle {
		...
		for (int i = 0; i < points.size(); i++) {
			...
			if (!c.contains(q)) {
				if (c.r == 0)
					...
					c = makeccTwoPoints(points.subList(0, i + 1), p, q);
			}
		}
		return c;
	}
	...
	private static Circle makeccTwoPoints(List<Point> points, Point p, Point q) {
		...
		for (Point r : points) {
			...
			// Form a circumcc and classify it on left or right side
			double cross = pq.cross(r.subtract(p));
			Circle c = makeCircumscribedCircle(p, q, r);
			if (c == null)
				continue;
	...
	static Circle makeCircumscribedCircle(Point a, Point b, Point c) {
...
final class Circle {
	...
	public List<Point> points_list = new ArrayList<>();
	public final Point c;   // Center
	public final double r;  // Radius
	public final double rounded_r;


	public Circle(Point c, double r) {
		this.c = c;
		this.r = r;
		this.rounded_r = Math.ceil(r);
	}
	...
	public String toString() {
		return String.format("c(%g, %g), %g) Max range=%f Meters", c.x, c.y, r,rounded_r);
	}
